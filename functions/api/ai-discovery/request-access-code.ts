...
export async function handleRequestAccessCode(
  ...
  try {
    ...
    // Generate secure 4-digit OTP challenge (stateless TOTP + HMAC signed challenge token)
    const identifier = cleanEmail || cleanPhone;
    const secret = getSessionSecret(env);
    const directCode = await generateTimeWindowOtp(cleanEmail || identifier, secret, 15);
