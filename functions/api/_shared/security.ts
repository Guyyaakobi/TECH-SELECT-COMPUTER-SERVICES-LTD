...
export function getSessionSecret(env?: any): string {
  ...
  const secret =
    ...
    (typeof process !== "undefined" && (process.env?.SESSION_SECRET || process.env?.CLIENT_SECRET)) ||
    "tech-select-sec-hmac-98124-prod-auth";
  return secret.trim();
}
...
function base64UrlEncode(str: string): string {
  // UTF-8 safe base64 encoding (supports Hebrew, Arabic, Emoji, Unicode)
...
export async function verifyTimeWindowOtp(code: string, identifier: string, secret: string, windowMinutes = 15): Promise<boolean> {
  ...
  // Check current window, previous 2 windows, and next window (allows up to 45 mins + clock skew)
  for (const w of [currentWindow, currentWindow - 1, currentWindow - 2, currentWindow + 1]) {
    const digestBuffer = await crypto.subtle.digest(
      "SHA-256",
 ...
 * Contains: hash(code + salt), clean code, email/phone, and expiration (30 mins).
 * Cryptographically signed with HMAC-SHA256 so it cannot be forged or tampered with.
 */
export async function createOtpChallengeToken(code: string, identifier: string, secret: string): Promise<string> {
  ...
  const codeDigestBuffer = await crypto.subtle.digest(
    "SHA-256",
    enc.encode(`${cleanCode}:${cleanId}:${secret}`)
  );
...
export async function verifyOtpChallenge(
  ...
  try {
    ...
    // 1. Direct HMAC-verified code matching (100% reliable across any isolate/region)
    if (payload.c && String(payload.c).trim() === cleanCode) {
      return { valid: true };
    }

    // 2. Hash-based matching with payload ID
    ...
    // 3. Secondary check with provided identifier if different
    if (identifier && identifier.toLowerCase().trim() !== cleanId) {
      const altDigest = await crypto.subtle.digest(
        "SHA-256",
        enc.encode(`${cleanCode}:${identifier.toLowerCase().trim()}:${secret}`)
      );
