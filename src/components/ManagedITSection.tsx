...
export const ManagedITSection: React.FC = () => {
  ...
  return (
    <section id="managed" className={`pt-36 sm:pt-40 pb-14 relative overflow-hidden transition-colors duration-300 bg-transparent ${
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            {/* Clean Typography Checklist (No heavy boxes) */}
            <div className={`space-y-3.5 pt-2 animate-linear-cta divide-y ${
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            <div className={`relative p-7 rounded-2xl flex flex-col items-center text-center overflow-hidden border backdrop-blur-md transition-all duration-300 ${
              isDark 
                ? 'bg-white/[0.02] border-white/[0.08]' 
                : 'bg-white/70 border-slate-200/80 shadow-xs'
            }`}>
              <div className={`relative z-10 w-20 h-20 rounded-full p-1 mb-6 flex items-center justify-center border ${
                ...
                <div className={`w-full h-full rounded-full flex flex-col items-center justify-center p-2 ${
                  isDark ? 'bg-[#05070c]' : 'bg-white shadow-xs'
                }`}>
                  <Server className="w-6 h-6 text-cyan-400" />
                  <span className={`text-[9px] font-sans font-bold uppercase tracking-widest mt-1 ${
                    isDark ? 'text-white' : 'text-slate-900'
                  }`}>Managed IT</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2.5 w-full text-xs relative z-10 font-medium">
                <div className={`p-3 rounded-xl flex items-center gap-2 transition-all ${
                  isDark ? 'bg-white/[0.03] text-slate-200 hover:bg-white/[0.06]' : 'bg-slate-50 text-slate-800 hover:bg-slate-100'
                }`}>
            ...
            {/* Cost Comparison Box */}
            <div className={`p-6 rounded-2xl border space-y-2.5 transition-all ${
              isDark 
              ...
              <p className="text-xs sm:text-sm leading-relaxed">
                {isHe ? (
                  <>
                    ...
                    <strong className={isDark ? "text-cyan-300 font-bold" : "text-blue-700 font-bold"}>שירות IT מנוהל ו-FinOps ב-TECH-SELECT:</strong> תשלום חודשי קבוע ומוזל, ניקוי רישיונות ומנויים כפולים, וחיבור ישיר של השכבה הפיזית והחומרה בארגון.
                  </>
                ) : (
                  <>
                    Full-time in-house IT Director: ~$10,000/mo + overhead.<br />
                    <strong className={isDark ? "text-cyan-300 font-bold" : "text-blue-700 font-bold"}>TECH-SELECT Managed IT & FinOps:</strong> Flat monthly retainer, elimination of redundant licensing, and direct physical infrastructure integration.
                  </>
                )}
              </p>
            </div>
