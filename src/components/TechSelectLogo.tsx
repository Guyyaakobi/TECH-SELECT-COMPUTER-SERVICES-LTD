import React from 'react';

interface LogoProps {
  size?: 'sm' | 'md' | 'lg' | 'xl';
  showSubtag?: boolean;
  theme?: 'light' | 'dark';
}

export const TechSelectLogo: React.FC<LogoProps> = ({ size = 'md', showSubtag = true, theme = 'dark' }) => {
  const sizeClasses = {
    sm: { laptop: 'w-8 h-5.5 sm:w-9 sm:h-6', title: 'text-[15px] sm:text-[17px] xl:text-[18.5px]', sub: 'text-[9.5px] sm:text-[10.5px]' },
    md: { laptop: 'w-10 h-7', title: 'text-lg sm:text-xl xl:text-2xl', sub: 'text-[11.5px] sm:text-xs' },
    lg: { laptop: 'w-13 h-9', title: 'text-2xl sm:text-3xl', sub: 'text-xs sm:text-sm' },
    xl: { laptop: 'w-16 h-11', title: 'text-3xl sm:text-4xl', sub: 'text-sm sm:text-base' },
  };

  const currentSize = sizeClasses[size];
  const textColor = theme === 'light' ? 'text-slate-900' : 'text-slate-100';
  const subColor = theme === 'light' ? 'text-slate-700' : 'text-slate-300';
  const strokeColor = theme === 'light' ? '#0f172a' : '#f8fafc';

  // Subtle drop shadow tailored for light and dark themes
  const iconShadow = theme === 'light' 
    ? 'drop-shadow-[0_2px_4px_rgba(15,23,42,0.14)]' 
    : 'drop-shadow-[0_2px_8px_rgba(0,0,0,0.65)]';

  const textShadow = theme === 'light'
    ? '[text-shadow:_0_1px_2px_rgba(15,23,42,0.12)]'
    : '[text-shadow:_0_1.5px_4px_rgba(0,0,0,0.65)]';

  return (
    <div className="flex items-center gap-3.5 group cursor-pointer select-none dir-ltr text-left">
      {/* Laptop Icon SVG - Matches User's Exact Logo Artwork with Subtle Shadow */}
      <div className="relative shrink-0 flex items-center justify-center transition-transform duration-300 group-hover:scale-105">
        <svg
          viewBox="0 0 100 65"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className={`${currentSize.laptop} transition-all duration-300 ${iconShadow}`}
        >
          {/* Laptop Screen Outer Border */}
          <rect
            x="12"
            y="6"
            width="76"
            height="46"
            rx="4"
            stroke={strokeColor}
            strokeWidth="6"
            fill="none"
          />

          {/* Inner Screen Display Surface */}
          <rect
            x="17"
            y="11"
            width="66"
            height="36"
            fill="none"
          />

          {/* Laptop Base Hinge / Tray */}
          <path
            d="M2 56C2 54 4 52 6 52H94C96 52 98 54 98 56V57C98 59 96 60 94 60H6C4 60 2 59 2 57V56Z"
            fill={strokeColor}
          />

          {/* Trackpad / Finger Latch Cutout */}
          <path
            d="M40 52H60V54C60 55 59 56 58 56H42C41 56 40 55 40 54V52Z"
            fill={theme === 'light' ? '#ffffff' : '#0b0c10'}
          />
        </svg>
      </div>

      {/* Official Serif Typography Brand Name */}
      <div className={`flex flex-col text-left font-serif leading-tight ${textShadow}`}>
        {/* Top Title: Tech Select */}
        <span className={`${currentSize.title} font-serif tracking-tight font-normal ${textColor}`}>
          Tech Select
        </span>

        {/* Subtitle: computer services LTD */}
        {showSubtag && (
          <span className={`${currentSize.sub} font-serif tracking-normal font-normal ${subColor} opacity-90 -mt-0.5`}>
            computer services LTD
          </span>
        )}
      </div>
    </div>
  );
};
