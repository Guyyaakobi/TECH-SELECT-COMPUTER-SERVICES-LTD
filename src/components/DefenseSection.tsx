      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-sans font-bold uppercase tracking-wider border ${
            isDark 
              ? 'bg-white/[0.06] border-white/[0.14] text-slate-100' 
          ...
          <h2 className={`text-3xl sm:text-5xl font-extrabold font-heading tracking-tight leading-tight ${
            isDark ? 'text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.9)]' : 'text-slate-950'
          }`}>
            {isHe ? (
              <>
          ...
          <p className={`text-base sm:text-lg leading-relaxed font-normal ${
            isDark ? 'text-slate-200 drop-shadow-[0_2px_8px_rgba(0,0,0,0.8)]' : 'text-slate-700'
          }`}>
            {isHe
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {defenseHighlights.map((item, index) => {
            ...
            return (
              <SpotlightCard
                key={index}
                className={`p-6 flex flex-col justify-between ${isHe ? 'text-right' : 'text-left'}`}
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${
                      isDark ? 'bg-white/[0.04] text-cyan-400' : 'bg-blue-50 text-blue-700'
                    }`}>
                  ...
                  {item.checkmark && (
                    <div className={`mt-4 border-s-2 ps-3 py-1 flex items-start gap-2 text-[11.5px] leading-relaxed ${
                      isDark 
        ...
        <div className={`p-8 sm:p-10 rounded-3xl relative overflow-hidden border transition-all duration-300 backdrop-blur-md ${
          isDark
          ...
          <div className={`grid lg:grid-cols-12 gap-8 items-center ${isHe ? 'text-right' : 'text-left'} relative z-10`}>
            <div className="lg:col-span-8 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs font-sans font-semibold ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
