  ...
  return (
    <div className={`rounded-2xl border transition-all relative overflow-hidden shadow-sm ${
      ...
      <div className="px-6 pt-6 pb-4 flex flex-col sm:flex-row items-center justify-between gap-3 border-b border-inherit">
        <div className="text-center sm:text-right">
          ...
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">
            {isHe 
              ? 'אינטגרציה מלאה, רישוי רשמי, הקשחה ותמיכה ישירה מול היצרנים המובילים בעולם' 
              : 'End-to-end integration, tier-1 licensing, security hardening and certified operations'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 text-xs text-slate-600 dark:text-slate-300 bg-white/70 dark:bg-white/5 px-3.5 py-1.5 rounded-full border border-slate-200/80 dark:border-white/10 shadow-xs">
          ...
          <span className="font-semibold">{isHe ? 'מוקד תמיכה וניהול 24/7' : '24/7 Certified Operations'}</span>
        </div>
      </div>
        ...
        <div className="animate-marquee gap-4 sm:gap-5 mb-4">
          {infiniteTrack1.map((item, idx) => (
            <div
              ...
              className={`flex items-center gap-4 px-5 py-3.5 sm:px-6 sm:py-4 rounded-xl border transition-all shrink-0 cursor-default select-none shadow-xs hover:scale-[1.02] ${
                isDark 
                  ? 'bg-[#070a14] border-white/10 hover:border-blue-500/50 hover:bg-[#0e1730]' 
                  : 'bg-white border-slate-200/90 hover:border-blue-400 hover:bg-blue-50/50'
              }`}
            >
              {/* Logo SVG in dedicated box */}
              <div className={`shrink-0 w-12 h-12 sm:w-14 sm:h-14 rounded-xl flex items-center justify-center p-2 ${
      ...
      <div className={`px-6 py-3 text-center text-xs sm:text-sm font-medium border-t ${
        ...
        {isHe 
          ? '• אפיון, רישוי מרוכז וניהול שוטף של כל מערכות החומרה והתוכנה תחת חשבונית ומוקד תמיכה אחד •'
