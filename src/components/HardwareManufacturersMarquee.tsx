...
export const HardwareManufacturersMarquee: React.FC<HardwareManufacturersMarqueeProps> = ({ embedded = false }) => {
  ...
  return (
    <div className={`transition-all relative overflow-hidden ${
      embedded 
        ? `mt-8 pt-6 border-t ${isDark ? 'border-white/10' : 'border-slate-200'}` 
        : `rounded-2xl border my-6 sm:my-8 shadow-sm ${isDark ? 'bg-[#0a0f1d] border-white/10' : 'bg-slate-50/90 border-slate-200'}`
    }`}>
      {/* Header bar */}
      <div className={`flex flex-col sm:flex-row items-center justify-between gap-3 pb-4 border-b ${
        embedded ? 'px-1 sm:px-2 border-inherit' : 'px-6 pt-6 border-inherit'
      }`}>
        <div className="text-center sm:text-right">
          <div className="text-sm sm:text-base font-extrabold uppercase tracking-wider text-blue-600 dark:text-cyan-400">
            {isHe ? 'יצרנים ופתרונות חומרה רשמיים' : 'OFFICIAL OEM HARDWARE ECOSYSTEM'}
          </div>
        ...
        <div className="flex items-center gap-2.5 text-xs text-slate-600 dark:text-slate-300 bg-white/70 dark:bg-white/5 px-3.5 py-1.5 rounded-full border border-slate-200/80 dark:border-white/10 shadow-xs shrink-0">
