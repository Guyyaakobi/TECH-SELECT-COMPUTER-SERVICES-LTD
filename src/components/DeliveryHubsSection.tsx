...
export const DeliveryHubsSection: React.FC<DeliveryHubsSectionProps> = ({
  ...
  return (
    <div id="about" className={`min-h-screen pb-20 transition-colors duration-300 ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>
      ...
      <div className="relative min-h-[620px] sm:min-h-[700px] flex flex-col items-center justify-center pt-36 pb-12 sm:pt-40 sm:pb-16 px-4 sm:px-6 mb-12 overflow-hidden border-b border-slate-200 dark:border-white/10">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
          ...
          <section className="mb-14">
              ...
              <div className="relative aspect-[16/9] sm:aspect-[21/9] w-full overflow-hidden">
                ...
                <div className="absolute bottom-3 start-3 end-3 sm:bottom-4 sm:start-4 sm:end-4 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium backdrop-blur-md bg-emerald-950/70 text-emerald-300 border border-emerald-500/30">
                    ...
                    <span>{isHe ? 'צוות הנדסה פנימי Tier-3/4 זמין וייעודי' : 'Dedicated In-House Tier-3/4 Engineering'}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
              ...
              <div className={`p-6 rounded-2xl border transition-all backdrop-blur-xl ${
                isDark 
                  ? 'bg-[#091122]/82 border-white/[0.14] shadow-[0_16px_45px_0_rgba(0,0,0,0.45),inset_0_1px_0_0_rgba(255,255,255,0.15)] hover:border-cyan-400/50' 
                  : 'bg-white/88 border-slate-200/90 shadow-[0_16px_40px_0_rgba(37,99,235,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)] hover:border-blue-400/60'
              }`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {coreValues.map((val) => {
                ...
                return (
                  <div 
                    key={val.id}
                    className={`p-5 rounded-2xl border transition-all flex flex-col justify-between backdrop-blur-xl ${
                      isDark 
                        ? 'bg-[#091122]/82 border-white/[0.14] hover:border-cyan-400/50 hover:bg-[#0f1830]/90 shadow-[0_12px_35px_0_rgba(0,0,0,0.45),inset_0_1px_0_0_rgba(255,255,255,0.15)]' 
                        : 'bg-white/88 border-slate-200/90 hover:border-blue-400/60 hover:bg-white shadow-[0_12px_35px_0_rgba(37,99,235,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
                    }`}
                  >
                    <div>
                      {/* Icon & Title */}
        ...
        {(activeTab === 'all' || activeTab === 'team') && (
          <section className="mb-14">
            ...
            {/* CEO SPOTLIGHT CARD - Clean, Crisp High-Resolution Bento Presentation */}
            <div className={`p-6 sm:p-8 rounded-2xl border mb-6 transition-all backdrop-blur-xl ${
              isDark 
              ...
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                ...
                {/* Ultra-Crisp Portrait Avatar Container */}
                <div className="relative shrink-0">
                  <div className="w-32 h-32 sm:w-36 sm:h-36 rounded-2xl overflow-hidden border-2 border-blue-500/30 dark:border-cyan-500/30 shadow-xl relative flex items-center justify-center ring-4 ring-blue-500/10 dark:ring-cyan-500/10 bg-slate-900">
                    <img
                      src="/guy-yaakobi.jpg"
                      alt={isHe ? 'גיא יעקובי - מייסד ומנכ"ל | ארכיטקט מערכות ראשי' : 'Guy Yaakobi - Founder & CEO'}
                  ...
                  <div className="mt-2 text-center text-[10px] font-mono font-bold px-2 py-0.5 rounded-full border shadow-xs bg-blue-500/10 border-blue-500/30 text-blue-600 dark:text-cyan-400">
                ...
                <div className="flex-1 text-center sm:text-right">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
                    <div>
                      ...
                      <p className="text-xs sm:text-sm text-blue-600 dark:text-cyan-400 font-bold mt-0.5">
                        {isHe ? 'מייסד ומנכ"ל | ארכיטקט מערכות ראשי' : 'Founder & CEO | Principal Enterprise Architect'}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center justify-center sm:justify-end gap-2">
                      <a
                        ...
                        className={`px-3 py-1.5 rounded-xl border text-xs font-semibold inline-flex items-center gap-1.5 transition-colors ${
                          isDark ? 'bg-white/5 hover:bg-[#0077b5]/20 hover:border-[#0077b5]/40 border-white/10 text-slate-200' : 'bg-white hover:bg-slate-100 border-slate-200 text-slate-700'
                        }`}
                        title="LinkedIn"
                      >
                      ...
                      {guy.email && (
                        <a
                          ...
                          className={`px-3 py-1.5 rounded-xl border text-xs font-semibold inline-flex items-center gap-1.5 transition-colors ${
                            isDark ? 'bg-white/5 hover:bg-cyan-500/20 hover:border-cyan-500/40 border-white/10 text-slate-200' : 'bg-white hover:bg-slate-100 border-slate-200 text-slate-700'
                          }`}
                          title={guy.email}
                        >
                  ...
                  {/* Bio Manifesto Quote */}
                  <div className={`p-4 rounded-xl border relative mb-3 ${
                    isDark ? 'bg-white/[0.03] border-white/10 text-slate-100' : 'bg-slate-50/80 border-slate-200/80 text-slate-800'
                  }`}>
                    <p className="text-xs sm:text-sm leading-relaxed font-sans font-medium italic">
                      {isHe 
                        ? '״טכנולוגיה טובה מתחילה באנשים שלוקחים אחריות. אנחנו עובדים בגובה העיניים, מלווים את הנהלת הארגון כשותף טכנולוגי, ומנהלים את מערך ה-IT מקצה לקצה — מתכנון ויישום ועד תמיכה שוטפת. המטרה פשוטה: מערכות יציבות, עבודה רציפה ושקט תפעולי.״'
                        : '“Great technology starts with people who take ownership. We communicate at eye level, accompany executive leadership as a trusted technology partner, and manage corporate IT end-to-end — from architecture and deployment to ongoing mission-critical support. The goal is simple: stable systems, continuous workflow, and operational peace of mind.”'}
                    </p>
                  </div>

                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                    <span className="px-2.5 py-0.5 rounded-md bg-blue-500/10 text-blue-600 dark:text-cyan-400 text-[11px] font-medium border border-blue-500/20">
                      {isHe ? 'שותפות טכנולוגית (vCIO)' : 'Strategic vCIO Practice'}
                    </span>
                    <span className="px-2.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-[11px] font-medium border border-emerald-500/20">
                      {isHe ? 'ספק מורשה משרד הביטחון' : 'MoD Authorized Supplier'}
                    </span>
                    <span className="px-2.5 py-0.5 rounded-md bg-purple-500/10 text-purple-600 dark:text-purple-400 text-[11px] font-medium border border-purple-500/20">
                      {isHe ? 'אחריות Turnkey מלאה' : 'Full Turnkey Ownership'}
                    </span>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
              ...
              <div className={`p-6 rounded-2xl border transition-all backdrop-blur-xl ${
                ...
                <p className={`text-xs sm:text-sm leading-relaxed ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                  {isHe 
                    ? 'מענה הנדסי ישיר ומקצועי, ניהול שרתים ותחנות קצה, גיבויים שוטפים וליווי vCIO צמוד.'
                    : 'Dedicated engineering support, proactive server management, endpoints, regular backups, and executive vCIO.'}
                </p>
              </div>
        ...
        {/* ----------------------------------------------------------------------- */}
        <div className={`mt-6 p-6 sm:p-8 rounded-2xl border text-center relative overflow-hidden transition-all backdrop-blur-xl ${
          isDark 
            ? 'bg-[#091122]/82 border-white/[0.14] shadow-[0_16px_50px_0_rgba(0,0,0,0.55),inset_0_1px_0_0_rgba(255,255,255,0.15)]' 
            : 'bg-white/88 border-slate-200/90 shadow-[0_16px_45px_0_rgba(37,99,235,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}>
          ...
          <p className={`text-xs sm:text-sm max-w-xl mx-auto mb-6 leading-relaxed ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
            {isHe
