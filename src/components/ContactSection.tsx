...
export const ContactSection: React.FC<ContactSectionProps> = ({ onOpenPrivacy }) => {
  ...
  return (
    <section id="contact" className={`pt-36 sm:pt-40 pb-14 relative overflow-hidden transition-colors duration-300 bg-transparent ${
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-start animate-linear-cta">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* Unified Info Column */}
            <div className={`p-7 sm:p-8 space-y-8 rounded-3xl border transition-all backdrop-blur-md ${
              isDark 
              ...
              <div className="space-y-4">
                ...
                <div className={`space-y-4 text-xs font-medium ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                ...
                <div className="pt-2 flex items-center gap-3">
                  <a
                    ...
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-full text-xs font-sans font-semibold transition-colors ${
                      isDark 
                        ? 'bg-white/[0.04] hover:bg-white/[0.08] text-cyan-300' 
                        : 'bg-slate-100 hover:bg-slate-200 text-cyan-800'
                    }`}
                  >
              ...
              {/* Operating Hours */}
              <div className={`pt-6 border-t space-y-3 ${isDark ? 'border-white/[0.08]' : 'border-slate-200'}`}>
              ...
              {/* Regional Coverage & Deployment Hubs */}
              <div className={`pt-6 border-t space-y-3 ${isDark ? 'border-white/[0.08]' : 'border-slate-200'}`}>
                <div className={`flex items-center gap-2 text-xs font-bold uppercase tracking-wider font-sans ${
                  isDark ? 'text-white' : 'text-slate-900'
                }`}>
                  <MapPin className="w-4 h-4 text-blue-400" />
                  <span>{isHe ? 'פריסה גיאוגרפית ומוקדי הגעה' : 'Regional Field Coverage'}</span>
                </div>
                <p className={`text-xs leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  {isHe
                    ? 'צוות המהנדסים של TECH-SELECT מעניק זמני תגובה והגעה מהירים (SLA) עם פריסה ייעודית באתר הלקוח:'
                    : 'TECH-SELECT engineering squads provide rapid on-site field response across key business regions:'}
                </p>
                <div className="space-y-3 text-xs">
                  <div className={`border-s-2 ps-3.5 py-1 ${
                    isDark ? 'border-cyan-400/50' : 'border-blue-600'
                  }`}>
                    <span className="font-bold block text-slate-900 dark:text-white">
                      {isHe ? 'אזור הצפון והגליל' : 'Northern District & Galilee'}
                    </span>
                    <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
                      {isHe 
                        ? 'חיפה (פארק מת״מ), פארק יקנעם, כרמיאל, משגב ומפעלי ביטחון ותעשייה בצפון'
                        : 'Haifa (Matam), Yokneam Tech Park, Karmiel & Northern Defense Manufacturers'}
                    </span>
                  </div>

                  <div className={`border-s-2 ps-3.5 py-1 ${
                    isDark ? 'border-blue-400/50' : 'border-blue-600'
                  }`}>
                    <span className="font-bold block text-slate-900 dark:text-white">
                      {isHe ? 'אזור השרון' : 'Sharon District'}
                    </span>
                    <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
                      {isHe 
                        ? 'הרצליה פיתוח, פארק הייטק רעננה, נתניה וקריית ספיר/פולג, כפר סבא והוד השרון'
                        : 'Herzliya Pituach, Raanana Tech Zone, Netanya Poleg, Kfar Saba & Hod HaSharon'}
                    </span>
                  </div>

                  <div className={`border-s-2 ps-3.5 py-1 ${
                    isDark ? 'border-indigo-400/50' : 'border-blue-600'
                  }`}>
                    <span className="font-bold block text-slate-900 dark:text-white">
                      {isHe ? 'מרכז וגוש דן' : 'Central & Gush Dan'}
                    </span>
                    <span className={isDark ? 'text-slate-400' : 'text-slate-600'}>
                      {isHe 
                        ? 'מטה תל אביב, פתח תקווה, בורסת רמת גן, רחובות, ופריסה ארצית'
                        : 'Tel Aviv HQ, Petah Tikva, Ramat Gan, Rehovot & Nationwide Coverage'}
                    </span>
                  </div>
                </div>
              </div>
          ...
          {/* Contact Form */}
          <div className={`lg:col-span-7 p-7 sm:p-8 rounded-3xl border transition-all backdrop-blur-md ${
            isDark 
              ? 'bg-white/[0.02] border-white/[0.08]' 
              : 'bg-white/80 border-slate-200/80 shadow-xs'
          }`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className={`w-full rounded-xl px-4 py-3 text-xs sm:text-sm outline-none border transition-colors ${
                      isDark 
                        ? 'bg-white/[0.05] text-white placeholder-slate-400 border-white/10 focus:border-cyan-400' 
                        : 'bg-white text-slate-900 placeholder-slate-400 border-slate-300 focus:border-blue-600'
                    }`}
                  />
                </div>
                ...
                <div className="grid sm:grid-cols-3 gap-3">
                  <div>
                    <label className={`block text-xs font-medium mb-1 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                    ...
                    <input
                      ...
                      className={`w-full rounded-xl px-4 py-3 text-xs sm:text-sm outline-none border transition-colors font-sans ${
                        isDark 
                          ? 'bg-white/[0.05] text-white placeholder-slate-400 border-white/10 focus:border-cyan-400' 
                          : 'bg-white text-slate-900 placeholder-slate-400 border-slate-300 focus:border-blue-600'
                      }`}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className={`w-full rounded-xl px-4 py-3 text-xs sm:text-sm outline-none border transition-colors resize-none ${
                      isDark 
                        ? 'bg-white/[0.05] text-white placeholder-slate-400 border-white/10 focus:border-cyan-400' 
                        : 'bg-white text-slate-900 placeholder-slate-400 border-slate-300 focus:border-blue-600'
                    }`}
