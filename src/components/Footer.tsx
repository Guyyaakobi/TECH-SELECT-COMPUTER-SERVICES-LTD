        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12 pb-12 border-b ${
          ...
          <div className={`lg:col-span-2 space-y-3 text-xs ${isHe ? 'text-right' : 'text-left'}`}>
            ...
            <ul className={`space-y-2 font-medium ${
              ...
              <li><a href="#services" className={`transition-colors ${isDark ? 'hover:text-cyan-300' : 'hover:text-blue-600'}`}>{isHe ? 'שירותים מנוהלים (MSP)' : 'Tech & IT Solutions'}</a></li>
          ...
          <div className={`lg:col-span-3 space-y-3 text-xs ${isHe ? 'text-right' : 'text-left'}`}>
            ...
            <div className="pt-2">
              <div className={`inline-flex items-center gap-2.5 px-3.5 py-2 rounded-2xl border text-xs font-sans ${
                isDark 
          ...
          <div className={`lg:col-span-3 space-y-3 text-xs ${isHe ? 'text-right' : 'text-left'}`}>
            ...
            <div className={`pt-2 text-[11px] ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
              <span className={`font-bold block mb-1 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                {isHe ? 'שעות פעילות הנדסית:' : 'Business Hours:'}
              </span>
              <div className="font-sans">{isHe ? 'א\'-ה\': 08:30 - 18:00 | מעטפת חירום בהסכם SLA' : 'Sun-Thu: 08:30 - 18:00 | Emergency SLA Coverage'}</div>
            </div>
          </div>
