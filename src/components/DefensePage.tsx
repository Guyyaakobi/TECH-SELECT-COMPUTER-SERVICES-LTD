...
export const DefensePage: React.FC<DefensePageProps> = ({ onBackToHome }) => {
  ...
  return (
    <div className={`font-sans transition-colors duration-300 selection:bg-cyan-500 selection:text-white relative overflow-hidden bg-transparent ${
      ...
      {/* Main Defense Content - Padded to sit cleanly under navbar and breadcrumbs while background reaches y=0 */}
      <div className="pt-36 sm:pt-40 pb-12 relative z-10">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-16">
          ...
          <div className="text-center max-w-4xl mx-auto space-y-6">
            ...
            <p className={`text-base sm:text-lg leading-relaxed max-w-3xl mx-auto font-normal animate-linear-subtitle ${
              ...
              {isHe ? (
                <>
                  ב-<strong className={isDark ? "text-white font-bold" : "text-slate-950 font-bold"}>TECH-SELECT (טק-סלקט בע"מ)</strong>, פיתחנו מענה מיועד וייחודי המותאם במיוחד לספקי מערכת הביטחון, חברות ביטחוניות ומתקנים מסווגים. אנו מעניקים שירותי IT, תשתיות רשת מבודדות (Air-Gap) וליווי רגולטורי על ידי צוות בעל סיווג ביטחוני בתוקף.
                </>
              ) : (
                <>
                  At <strong className={isDark ? "text-white font-bold" : "text-slate-950 font-bold"}>TECH-SELECT LTD</strong>, we provide a specialized division tailored for defense contractors, military suppliers, and classified facilities. We deliver isolated Air-Gap networks, hardware supply, and security compliance led by cleared engineers.
                </>
              )}
            </p>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    {item.checkmark && (
                      <div className={`mt-4 border-s-2 ps-3 py-1 flex items-start gap-2 text-xs font-medium leading-relaxed ${
                        isDark 
                  ...
                  <div className={`mt-6 pt-4 flex items-center justify-between text-[11px] font-mono border-t ${
                    isDark ? 'border-white/[0.08] text-slate-400' : 'border-slate-100 text-slate-500'
                  }`}>
          ...
          <div className={`rounded-3xl p-8 sm:p-10 space-y-8 border transition-all backdrop-blur-md ${
            isDark ? 'bg-white/[0.02] border-white/[0.08]' : 'bg-white/80 border-slate-200/80 shadow-xs'
          }`}>
            <div className="pb-4">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                <div className={`p-2.5 w-fit rounded-xl ${
                  isDark ? 'bg-blue-500/10 text-blue-400' : 'bg-cyan-50 text-cyan-600'
                }`}>
          ...
          <div className="grid lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-6">
              <div className={`p-7 sm:p-8 rounded-3xl space-y-5 border transition-all backdrop-blur-md ${
                isDark 
                ...
                <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono font-bold ${
                  isDark ? 'bg-cyan-500/10 text-cyan-300' : 'bg-blue-50 text-blue-800'
                }`}>
                ...
                <div className={`space-y-3 pt-4 border-t text-xs font-medium font-sans ${isDark ? 'border-white/[0.08] text-slate-300' : 'border-slate-200 text-slate-800'}`}>
                  <a
