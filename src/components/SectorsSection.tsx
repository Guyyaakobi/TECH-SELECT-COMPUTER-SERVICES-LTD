...
export const SectorsSection: React.FC = () => {
  ...
  return (
    <section id="sectors" className={`pt-36 sm:pt-40 pb-14 relative overflow-hidden transition-colors duration-300 bg-transparent ${
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-12 gap-8 items-stretch">
            ...
            {SECTORS_DATA.map((sec) => {
              ...
              return (
                <button
                  ...
                  className={`p-3.5 rounded-xl text-right transition-all flex items-center gap-3 cursor-pointer ${
                    isSelected
                      ? isDark 
          ...
          <div className={`lg:col-span-7 p-6 sm:p-8 flex flex-col justify-between rounded-3xl border transition-all backdrop-blur-md ${
            isDark 
              ? 'bg-white/[0.02] border-white/[0.08]' 
              : 'bg-white/75 border-slate-200/80 shadow-xs'
          }`}>
            <div className="space-y-6">
              <div className={`flex items-center gap-3 pb-4 border-b ${isDark ? 'border-white/[0.08]' : 'border-slate-200'}`}>
                ...
                <div>
                  ...
                  <p className={`text-xs font-sans font-bold ${isDark ? 'text-cyan-400' : 'text-blue-700'}`}>{subtitle}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {challenges.map((c, idx) => (
                    <div key={idx} className="flex items-start gap-3 text-xs sm:text-sm font-normal">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {solutions.map((s, idx) => (
                    <div key={idx} className={`border-s-2 ps-3.5 py-1 flex items-start gap-2.5 text-xs sm:text-sm font-medium leading-relaxed ${
                      isDark 
                        ? 'border-cyan-500/60 text-cyan-200/95' 
                        : 'border-blue-600 text-blue-950'
                    }`}>
                      <CheckCircle2 className={`w-4 h-4 shrink-0 mt-0.5 ${isDark ? 'text-cyan-400' : 'text-blue-600'}`} />
                      <span>{s}</span>
                    </div>
                  ))}
                </div>
            ...
            <div className={`pt-6 mt-6 border-t flex justify-end ${isDark ? 'border-white/[0.08]' : 'border-slate-200'}`}>
              <a
                href="#contact"
