...
export const FloatingAIConcierge: React.FC<FloatingAIConciergeProps> = ({
  ...
  // Bubble Visibility: Hidden on initial site load, circle rolls in after 6 seconds,
  // then the text expands smoothly 1 second later (at 7 seconds).
  ...
  // Rotating Prompts: What alternates in the text ("מה שמתחלף בכתב")
  ...
  // Rotate placeholder text every 3.8 seconds, paused on hover
  useEffect(() => {
  ...
  }, [rotatingPrompts.length, isTriggerHovered]);
    ...
    <>
      ...
      {!isOpen && isBubbleVisible && (
        <div
          ...
          className="fixed bottom-6 right-3 sm:right-6 z-[9990] print:hidden pointer-events-auto"
        >
          <div
            ...
            className={`group relative flex items-center rounded-full cursor-pointer transition-all duration-700 ease-out shadow-2xl backdrop-blur-2xl border active:scale-98 select-none ${
              ...
              isDark
                ? 'bg-slate-900/95 hover:bg-slate-800/95 border-white/10 hover:border-sky-500/50 shadow-black/60 shadow-[0_10px_35px_rgba(0,0,0,0.5),0_0_20px_rgba(56,189,248,0.12)]'
          ...
          >
            ...
            {/* Dynamic Rotating Animated Text - Bubble dimensions NEVER change, text alternates gently with AnimatePresence */}
            <div
              className={`flex-1 min-w-0 transition-opacity duration-500 ease-out overflow-hidden ${
                isPillExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'
              }`}
            >
              <div className="flex items-center gap-1.5 h-5 overflow-hidden">
                <span className="font-bold text-[11px] sm:text-xs tracking-wider text-sky-500 dark:text-sky-400 shrink-0">
                  LET THE AI WORK
                </span>
      ...
      {isOpen && (
        <div
          ...
          className={`fixed z-[9999] flex flex-col overflow-hidden backdrop-blur-2xl animate-in fade-in duration-200 transition-all ${
            ...
            `lg:inset-auto lg:bottom-6 lg:right-6 lg:w-[460px] lg:max-w-[490px] lg:h-[660px] lg:max-h-[90vh] lg:rounded-3xl lg:border lg:shadow-2xl`
          } ${
            isDark
