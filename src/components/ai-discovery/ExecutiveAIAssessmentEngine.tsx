...
Click **"Generate Executive Report"** to produce the full blueprint.`;
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      console.warn("AI Report request failed", err);
    }

    clearInterval(timer);
    setReportError(isHe ? 'מערכת ה-AI חווה עומס או שגיאה. לא ניתן לייצר דוח בשלב זה, אנו עובדים על פתרון.' : 'The AI engine is currently unavailable or overloaded. Cannot generate report at this time.');
    setViewState('input');
  };
  ...
  if (!isAuthenticated) {
    return (
      <div className="w-full max-w-2xl mx-auto my-6 px-4" dir={isHe ? 'rtl' : 'ltr'}>
        <div className={`rounded-3xl border p-6 sm:p-9 transition-all relative overflow-hidden backdrop-blur-2xl ${
          isDark 
            ? 'bg-[#0a1122]/65 border-white/[0.12] text-slate-100 shadow-[0_16px_50px_0_rgba(0,0,0,0.6),inset_0_1px_0_0_rgba(255,255,255,0.14)]' 
            : 'bg-white/75 border-slate-200/80 text-slate-900 shadow-[0_16px_40px_0_rgba(15,23,42,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}>
  ...
  if (viewState === 'generating') {
    return (
      <div className="w-full max-w-2xl mx-auto my-12" dir={isHe ? 'rtl' : 'ltr'}>
        <div className={`p-8 sm:p-12 rounded-3xl border text-center space-y-6 transition-all backdrop-blur-2xl ${
          isDark 
            ? 'bg-[#0a1122]/65 border-white/[0.12] text-slate-100 shadow-[0_16px_50px_0_rgba(0,0,0,0.6),inset_0_1px_0_0_rgba(255,255,255,0.14)]' 
            : 'bg-white/75 border-slate-200/80 text-slate-900 shadow-[0_16px_40px_0_rgba(15,23,42,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6" dir={isHe ? 'rtl' : 'ltr'}>
      ...
      {reportError && (
        <div className="p-4 mb-4 rounded-xl border bg-red-500/10 border-red-500/30 text-red-600 dark:text-red-400 flex items-center gap-3 shadow-sm">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className={`flex items-center gap-3 p-2 px-3.5 rounded-2xl border transition-all backdrop-blur-xl ${
          isDark 
            ? 'bg-[#0a1122]/60 border-white/[0.12] shadow-[0_8px_24px_0_rgba(0,0,0,0.3),inset_0_1px_0_0_rgba(255,255,255,0.12)]' 
            : 'bg-white/75 border-slate-200/80 shadow-[0_4px_20px_0_rgba(15,23,42,0.05),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}>
      ...
      {/* ================= CHAT MODE ================= */}
      {mode === 'chat' && (
        <div className={`rounded-2xl border overflow-hidden flex flex-col h-[550px] max-h-[70vh] transition-all backdrop-blur-2xl ${
          isDark 
            ? 'bg-[#0a1122]/70 border-white/[0.12] shadow-[0_12px_45px_0_rgba(0,0,0,0.5),inset_0_1px_0_0_rgba(255,255,255,0.14)]' 
            : 'bg-white/80 border-slate-200/90 shadow-[0_12px_36px_0_rgba(15,23,42,0.08),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
        }`}>
      ...
      {mode === 'guided' && (
        <div className="space-y-6">
          ...
          <div className={`rounded-2xl border p-5 sm:p-6 space-y-3.5 transition-all backdrop-blur-xl ${
            isDark 
              ? 'bg-[#0a1122]/65 border-white/[0.12] text-slate-200 shadow-[0_8px_32px_0_rgba(0,0,0,0.37),inset_0_1px_0_0_rgba(255,255,255,0.12)]' 
              : 'bg-white/75 border-slate-200/80 text-slate-800 shadow-[0_8px_30px_0_rgba(15,23,42,0.05),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
          }`}>
...
Example: We are a 60-employee engineering and service firm using Priority ERP and SharePoint. Our main bottleneck is manual technical spec searches and purchase order data entry...`
          ...
          <div className={`p-4 rounded-2xl border grid grid-cols-1 sm:grid-cols-3 gap-3 backdrop-blur-xl ${
            isDark ? 'bg-[#0a1122]/55 border-white/[0.12] shadow-[0_4px_20px_0_rgba(0,0,0,0.3)]' : 'bg-white/70 border-slate-200/80 shadow-[0_4px_20px_0_rgba(15,23,42,0.04)]'
          }`}>
            <div>
