...
export const LiveMonitorWidget: React.FC = () => {
  ...
  return (
    <div className={`relative rounded-2xl border p-6 overflow-hidden group transition-all duration-200 backdrop-blur-xl ${
      isDark 
        ? 'bg-[#0a1122]/65 border-white/[0.12] text-slate-100 shadow-[0_12px_40px_0_rgba(0,0,0,0.45),inset_0_1px_0_0_rgba(255,255,255,0.12)]' 
        : 'bg-white/75 border-slate-200/90 text-slate-900 shadow-[0_12px_36px_0_rgba(15,23,42,0.06),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
    }`}>
      {/* Top Accent Line */}
      ...
      <div className="grid grid-cols-3 gap-3 my-5 relative z-10">
        <div className={`rounded-xl p-3.5 border text-center transition-all backdrop-blur-md ${
          isDark ? 'bg-white/[0.04] border-white/10 text-slate-100' : 'bg-white/70 border-slate-200/80 text-slate-900'
        }`}>
