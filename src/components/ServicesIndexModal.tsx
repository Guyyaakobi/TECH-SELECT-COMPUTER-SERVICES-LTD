...
export const ServicesIndexModal: React.FC<ServicesIndexModalProps> = ({
  ...
  const categories = [
    ...
    },
    {
      ...
      borderColor: 'border-purple-500/30',
      activeGlow: 'shadow-[0_0_25px_rgba(168,85,247,0.25)]',
      descHe: 'ניהול IT היקפי בריטיינר קבוע, מענה הנדסי ישיר ופרויקטי הבראת תשתיות טכנולוגיות.',
      descEn: 'Proactive retainer support, direct senior engineering access & complete infrastructure remediation.',
      statsHe: 'מענה ישיר | SLA מהיר | יישור קו',
      statsEn: 'Direct Support | SLA Metrics | Remediation',
      itemsHe: [
        'מענה הנדסי ישיר ומהיר ע"י מהנדסי IT בכירים (ללא מוקדנים)',
        'פרויקטי "יישור קו" והבראת תשתיות שנבנו טלאי על טלאי',
        'ניהול מלווה של מנהל IT ארגוני במודל ריטיינר קבוע ושקוף',
        'ניטור מערכות רציף, עדכוני אבטחה שוטפים ומניעת תקלות מראש',
        'בניית תיק אתר (Site Documentation) מפורט ושקוף לארגון',
        'התחייבות SLA לרמת שירות מהירה ומדידה לכל קריאה'
      ],
      itemsEn: [
        'Direct Access to Senior IT Engineers (No Junior Dispatchers)',
      ...
      brands: ['Enterprise SLA', 'Site Audit', 'Network Topology', 'Proactive Monitoring', 'Dedicated Engineer']
    },
    {
      id: 'hardware',
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/80 backdrop-blur-2xl overflow-y-auto p-2 sm:p-4 md:p-6 animate-in fade-in duration-300">
      ...
      <div className={`relative z-10 w-full max-w-6xl rounded-2xl overflow-hidden my-auto max-h-[92vh] flex flex-col transition-all duration-200 border backdrop-blur-2xl ${
        isDark 
          ? 'bg-[#0a1122]/85 border-white/[0.14] text-slate-100 shadow-[0_25px_90px_rgba(0,0,0,0.98),inset_0_1px_0_0_rgba(255,255,255,0.14)]' 
          : 'bg-white/90 border-slate-200/90 text-slate-900 shadow-[0_25px_90px_rgba(15,23,42,0.18),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
      }`}>
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2.5">
              ...
              <h2 className={`text-2xl sm:text-3xl md:text-4xl font-extrabold font-heading tracking-tight leading-tight ${
                ...
                {isHe ? (
                  <>
                    מפתח השירותים, התשתיות <span className="gemini-text-gradient">והשותפויות הבינלאומיות</span>
                  </>
                ) : (
                  <>
                    Enterprise Services & <span className="gemini-text-gradient">Global Tech Alliances</span>
                  </>
                )}
              </h2>
          ...
          <div className="grid gap-6">
            {filteredCategories.map((cat) => {
              ...
              return (
                <div
                  key={cat.id}
                  className={`group relative rounded-2xl p-6 sm:p-7 transition-all duration-300 space-y-5 overflow-hidden border backdrop-blur-xl ${
                    isDark 
                      ? 'bg-[#0a1122]/70 border-white/[0.12] hover:border-cyan-500/40 shadow-[0_8px_30px_rgba(0,0,0,0.5),inset_0_1px_0_0_rgba(255,255,255,0.1)]' 
                      : 'bg-white/75 border-slate-200/90 hover:border-blue-300 shadow-[0_8px_30px_rgba(15,23,42,0.06),inset_0_1px_0_0_rgba(255,255,255,0.95)]'
                  }`}
                >
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-1 relative z-10">
                    {items.map((item, idx) => (
                      <div
                        key={idx}
                        className={`flex items-start gap-2.5 p-3.5 rounded-xl border transition-all text-xs group/item backdrop-blur-md ${
                          isDark 
                            ? 'bg-white/[0.04] border-white/10 hover:border-cyan-500/30 text-slate-200' 
                            : 'bg-white/70 border-slate-200/80 hover:border-blue-300 text-slate-800'
                        }`}
                      >
