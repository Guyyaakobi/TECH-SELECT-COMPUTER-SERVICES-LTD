...
const SERVICE_MODAL_EN_DATA: Record<string, {
  ...
  'managed-it-systems': {
    ...
    features: [
      ...
      'Rapid-response live engineering support with guaranteed contractual response times',
...
export const ServiceModal: React.FC<ServiceModalProps> = ({ service, onClose, onSelectForQuote }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div
        dir={isHe ? 'rtl' : 'ltr'}
        className={`relative w-full max-w-3xl border rounded-2xl overflow-hidden max-h-[90vh] flex flex-col transition-all duration-200 backdrop-blur-2xl ${
          isDark 
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-sm">
          ...
          {(isHe ? service.highlightHe : service.highlightEn) && (
            <div className={`p-4 rounded-xl border flex items-start gap-3 backdrop-blur-md ${
              isDark 
                ? 'bg-cyan-950/30 border-cyan-500/30 text-cyan-200 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08)]' 
                : 'bg-blue-50/70 border-blue-200 text-blue-950 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.8)]'
            }`}>
          ...
          <div>
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              {modalFeatures.map((cap: string, idx: number) => (
                <div
                  key={idx}
                  className={`p-3.5 rounded-xl border flex items-start gap-3 transition-all backdrop-blur-md ${
                    isDark 
                      ? 'bg-white/[0.04] border-white/10 hover:border-cyan-500/40' 
                      : 'bg-white/70 border-slate-200/80 hover:border-blue-400'
                  }`}
                >
          ...
          <div className={`p-4 rounded-xl border flex flex-col sm:flex-row items-center justify-between gap-4 backdrop-blur-md ${
            isDark 
              ? 'bg-[#0b1322]/60 border-cyan-500/30 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.1)]' 
              : 'bg-blue-50/70 border-blue-200 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.8)]'
          }`}>
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-sans font-bold border ${
              ...
              {isHe ? 'אמנת שירות / SLA 99.99%' : 'Enterprise SLA 99.99%'}
            </span>
          </div>
        </div>
