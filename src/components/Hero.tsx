...
export const Hero: React.FC<HeroProps> = ({
  ...
  return (
    <section className="relative overflow-hidden transition-colors duration-300">
      ...
      {/* 1. Full-Screen Viewport Hero Fold (Background starts from top y=0 behind navbar) */}
      <div className="relative min-h-screen flex flex-col items-center justify-between pt-28 pb-12 sm:pt-36 sm:pb-16 px-4 sm:px-6 z-10 overflow-hidden">
        ...
        {/* Tel Aviv Ocean-View Architectural Enterprise Office Background (TECH-SELECT Headquarters with Team) */}
        <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden animate-linear-bg">
          <img
            ...
            loading="eager"
            decoding="async"
            className="w-full h-full object-cover object-[center_35%] scale-100 contrast-[1.08] brightness-[1.02] saturate-[1.12] transition-all duration-700 ease-out"
          />
          {/* Crisp, Balanced Tint Overlay allowing the office, sea horizon and team to be vividly visible while keeping cards ultra-readable */}
          <div className={`absolute inset-0 transition-colors duration-500 ${
            isDark
              ? 'bg-gradient-to-b from-[#080b12]/35 via-[#080b12]/30 to-[#080b12]/80'
              : 'bg-gradient-to-b from-white/35 via-white/30 to-[#f3f5f8]/85'
          }`} />

          {/* Soft Central Radial Tint to Keep Focal Text Razor Sharp while highlighting the vibrant backdrop */}
          <div className={`absolute inset-0 ${
            isDark
              ? 'bg-[radial-gradient(ellipse_at_center,rgba(8,11,18,0.12)_0%,rgba(8,11,18,0.60)_100%)]'
              : 'bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.12)_0%,rgba(243,245,248,0.60)_100%)]'
          }`} />

          {/* Seamless Bottom Fade Out into Bento Section */}
          <div className={`absolute bottom-0 inset-x-0 h-36 sm:h-48 bg-gradient-to-t ${
            isDark ? 'from-[#05070c] via-[#05070c]/80' : 'from-[#f3f5f8] via-[#f3f5f8]/80'
          } to-transparent pointer-events-none z-[2]`} />
        </div>

        {/* Gemini Delicate Sky-Blue Atmosphere Aura behind top headline */}
        <GeminiSkyGlow />
        ...
        <div className="relative z-10 text-center space-y-6 sm:space-y-8 max-w-5xl mx-auto my-auto py-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 pt-3 pb-3 animate-linear-cta">
            ...
            <a
              href="#contact"
              className="relative group inline-flex items-center gap-2.5 bg-blue-600 hover:bg-blue-700 active:scale-[0.98] text-white font-bold px-8 py-3.5 rounded-xl text-xs sm:text-sm transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md hover:-translate-y-0.5"
            >
              <span className="relative z-10">{isHe ? 'תיאום שיחת אבחון IT' : 'Schedule IT Assessment'}</span>
              {isHe ? (
                <ArrowLeft className="relative z-10 w-4 h-4 text-blue-100 group-hover:-translate-x-1 transition-transform duration-200" />
              ) : (
                <ArrowRight className="relative z-10 w-4 h-4 text-blue-100 group-hover:translate-x-1 transition-transform duration-200" />
              )}
            </a>
            ...
            <button
              ...
              className={`inline-flex items-center gap-2 px-7 py-3.5 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer group border hover:-translate-y-0.5 active:scale-[0.98] ${
                isDark 
                  ? 'bg-slate-900/90 hover:bg-slate-800 text-slate-200 hover:text-white border-white/15 shadow-sm'
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <span className={`text-xs font-sans font-semibold px-3 py-1 rounded-full border ${
                    isDark ? 'bg-white/[0.08] border-white/15 text-slate-200' : 'bg-slate-100 border-slate-200 text-slate-800'
                  }`}>
                    {isHe ? 'אחריות שוטפת ב-SLA' : 'Enterprise SLA Operations'}
                  </span>
                </div>

                <p className={`text-xs sm:text-sm leading-relaxed font-normal ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                  {isHe
                    ? 'ניהול כולל של תחנות עבודה, שרתים, משתמשים, סביבות Microsoft 365 ותקציב מחשוב. מענה הנדסי ישיר ומהיר, ניטור פרואקטיבי מתקדם, וליווי אסטרטגי שמעניק להנהלה שקט תעשייתי מלא.'
                    : 'End-to-end management of workstations, servers, users, M365 environments, and tech budgeting. Direct senior engineering response, proactive telemetry, and executive vCIO advisory.'}
                </p>

                {/* Visual Flow Steps - Clean Stepper Line (No mini boxes) */}
                <div className={`grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 divide-x ${
                  isHe ? 'divide-x-reverse' : ''
                } ${
                  isDark ? 'divide-white/[0.08]' : 'divide-slate-200'
                }`}>
                  <div className="py-1 px-2 space-y-1">
                    <span className="text-[10.5px] font-mono text-cyan-400 font-bold block">01. AUDIT</span>
                    <span className={`text-xs font-semibold block ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{isHe ? 'מיפוי וסקר' : 'Audit & Scoping'}</span>
                  </div>
                  <div className="py-1 px-2 space-y-1">
                    <span className="text-[10.5px] font-mono text-blue-400 font-bold block">02. STANDARDIZE</span>
                    <span className={`text-xs font-semibold block ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{isHe ? 'יישור קו' : 'Standardize'}</span>
                  </div>
                  <div className="py-1 px-2 space-y-1">
                    <span className="text-[10.5px] font-mono text-indigo-400 font-bold block">03. MONITOR</span>
                    <span className={`text-xs font-semibold block ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{isHe ? 'ניטור פרואקטיבי' : 'Proactive Telemetry'}</span>
                  </div>
                  <div className="py-1 px-2 space-y-1">
                    <span className="text-[10.5px] font-mono text-emerald-400 font-bold block">04. GOVERN</span>
                    <span className={`text-xs font-semibold block ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{isHe ? 'ניהול ו-vCIO' : 'vCIO Roadmap'}</span>
                  </div>
                </div>
              </div>

              <div className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold ${
                isDark ? 'border-white/[0.12] text-cyan-300' : 'border-slate-200 text-blue-700'
              }`}>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-sans font-bold px-2.5 py-0.5 rounded-full ${
                    isDark ? 'text-purple-300 bg-white/[0.06]' : 'text-purple-700 bg-purple-50'
                  }`}>
                    02. HIGH-SECURITY
                  </span>
                </div>
              ...
              {onNavigateToDefense ? (
                <button
                  ...
                  className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold cursor-pointer w-full transition-colors ${
                    isDark ? 'border-white/[0.08] text-purple-300 hover:text-purple-200' : 'border-slate-100 text-purple-700 hover:text-purple-900'
                  }`}
                >
                ...
                <div className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold ${
                  isDark ? 'border-white/[0.08] text-purple-300' : 'border-slate-100 text-purple-700'
                }`}>
            ...
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-sans font-bold px-2.5 py-0.5 rounded-full ${
                    isDark ? 'text-indigo-300 bg-white/[0.06]' : 'text-indigo-700 bg-indigo-50'
                  }`}>
                    03. PROJECTS
                  </span>
                </div>
              ...
              <div className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold ${
                isDark ? 'border-white/[0.08] text-indigo-300' : 'border-slate-100 text-indigo-700'
              }`}>
            ...
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-sans font-bold px-2.5 py-0.5 rounded-full ${
                    isDark ? 'text-blue-300 bg-white/[0.06]' : 'text-blue-700 bg-blue-50'
                  }`}>
                    04. SOFTWARE DEV
                  </span>
                </div>
              ...
              <div className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold ${
                isDark ? 'border-white/[0.08] text-blue-300' : 'border-slate-100 text-blue-700'
              }`}>
            ...
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-[10px] font-sans font-bold px-2.5 py-0.5 rounded-full ${
                    isDark ? 'text-emerald-300 bg-white/[0.06]' : 'text-emerald-700 bg-emerald-50'
                  }`}>
                    05. CLOUD & DR
                  </span>
                </div>
              ...
              <div className={`mt-6 pt-4 border-t flex items-center justify-between text-xs font-sans font-semibold ${
                isDark ? 'border-white/[0.08] text-emerald-300' : 'border-slate-100 text-emerald-700'
              }`}>
      ...
      {/* 3. "Who is TECH-SELECT for" Self-Selection Matrix (Open Editorial Layout, No nested boxes) */}
      <div className={`max-w-6xl mx-auto px-4 sm:px-6 relative z-10 py-12 sm:py-16 border-b ${
        ...
        {/* Clean Open Grid (No boxed cards) */}
        <div className={`grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 divide-y md:divide-y-0 ${
          isDark ? 'divide-white/[0.06]' : 'divide-slate-200'
        }`}>
          {fitItems.map((item, idx) => (
            <div
              key={idx}
              className={`flex items-start gap-4 ${idx > 0 ? 'pt-6 md:pt-0' : ''}`}
            >
              <div className={`p-1.5 rounded-full shrink-0 mt-1 ${
                isDark ? 'bg-cyan-500/10 text-cyan-400' : 'bg-blue-50 text-blue-600'
              }`}>
                <Check className="w-4 h-4" />
              </div>
              <div className={`space-y-1.5 ${isHe ? 'text-right' : 'text-left'}`}>
                <h4 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>{item.title}</h4>
                <p className={`text-xs sm:text-sm leading-relaxed ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{item.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`mt-12 pt-8 border-t flex flex-col sm:flex-row items-center justify-between gap-4 text-center ${
